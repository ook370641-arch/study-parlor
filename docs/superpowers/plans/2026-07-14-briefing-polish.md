...
### Task 10: Add E2E tests for import border lifecycle and annotation full lifecycle

**Files:**
- Create: `e2e/specs/article-annotations.spec.ts`
- Modify: `e2e/specs/anthropic-blog-ui.spec.ts`
- Modify: `e2e/helpers/selectors.ts`
- Modify: `src/components/anthropic/AnthropicArticleRow.tsx`（恢复 hidden testid）
- Modify: `src/components/article-assistant/ArticleAnnotations.tsx`（添加 testid）

**E2E scope rationale:** 五点中仅 Point 3（边框 + shimmer）和 Point 5（标注系统）需要 E2E。其余为纯 CSS/渲染变更，单元测试已覆盖。

- [ ] **Step 1: Add data-testid attributes to ArticleAnnotations**

Add testids for: ghost pen (`anno-ghost-pen`), note card (`anno-note-card`), textarea (`anno-note-textarea`), save button (`anno-save-button`), delete button (`anno-delete-button`), marker pen (`anno-marker-pen`), marked text (`anno-marked-text`).

- [ ] **Step 2: Restore hidden anthropic-article-saved testid in AnthropicArticleRow**

Add `<span data-testid="anthropic-article-saved" className="sr-only" />` when `article.isSaved` is true, to maintain backward compatibility with existing E2E selectors.

- [ ] **Step 3: Add annotation selectors to e2e/helpers/selectors.ts**
...
- [ ] **Step 4: Write E2E-8 — import border lifecycle (anthropic-blog-ui.spec.ts)**

```ts
test('E2E-8: 导入过程展示 shimmer 动画与彩色左边框状态', async ({ window }) => {
  // Navigate to Anthropic blog, wait for articles
  // Find unsaved article → verify no ember border class
  // Click unsaved → shimmer + spinner + "导入中…" visible
  // Wait for reader to open (import complete)
  // Verify article now has saved testid (ember border state)
})
```

- [ ] **Step 5: Write E2E-A1 — annotation full lifecycle (article-annotations.spec.ts, NEW)**

Full lifecycle: import article → select text (programmatic) → ghost pen appears → click ghost pen → note card opens → type note → save → marker visible in body → `.annotations.md` file written → switch source away and back → reopen article → marker persists → click marker → edit note → delete annotation → marker removed.

- [ ] **Step 6: Verify existing E2E tests not broken**

Check: `anthropic-article-saved` testid still works in `anthropic-blog.spec.ts` filter pattern. The `<span>` with `sr-only` class is present in DOM and locatable by Playwright.

- [ ] **Step 7: Run typecheck and unit tests**

```bash
npx tsc --noEmit  # Expected: PASS
npx vitest run    # Expected: 465 tests pass
```

- [ ] **Step 8: Build and run new E2E tests**

```bash
npm run build  # build the app first
npx playwright test --config=e2e/playwright.config.ts -g "E2E-8"
npx playwright test --config=e2e/playwright.config.ts -g "E2E-A1"
```

- [ ] **Step 9: Commit**

```bash
git add e2e/specs/article-annotations.spec.ts e2e/specs/anthropic-blog-ui.spec.ts e2e/helpers/selectors.ts src/components/anthropic/AnthropicArticleRow.tsx src/components/article-assistant/ArticleAnnotations.tsx
git commit -m "feat(e2e): add E2E tests for import border lifecycle and annotation full lifecycle"
```
