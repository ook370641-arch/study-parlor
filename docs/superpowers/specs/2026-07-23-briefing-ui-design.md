...
明确不做（YAGNI）：思绪边栏（B 方向，二期再议）、Newspaper 主题阅读态语录带（空态/星图内语录带以墨色呈现，见 §6/§4）、Anthropic/写作源星图动画、❧ 花饰、空态休眠星图（B 方案）。
...
全部为渲染层装饰，不进入任何 md 文件内容。Newspaper 主题阅读态不加语录带（空态/星图内语录带以墨色呈现，见 §6/§4）。
...
| 8 | Newspaper 主题 | 星图/空态换纸白配色；阅读态语录带与渐变遮罩不渲染（空态/星图内语录带以墨色呈现，见 §6/§4） |
