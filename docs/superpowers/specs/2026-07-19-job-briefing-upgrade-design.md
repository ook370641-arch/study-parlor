...
- **输入档案**：同迭代 1（filled + empty 两轮对照）。
- **发现的问题（人工审读 fixture 产物）**：
  1. **🔴 事件大面积过期**：43 条事件中大量 2023-2025 年内容（字节 2024 年 25 届宣讲会、智谱 2023 年宣讲会、腾讯 2025 年实习生招聘），迭代 1 的 prompt 时效过滤规则未被 LLM 有效执行。根因链：Tavily `days=7` 只约束索引时间 → 搜索 query 无年份锚点 → 提取 prompt 未注入当前日期 → 代码层无后置过滤。
  2. **🟡 投递链接是新闻页**：百度 · AI大模型评估产品经理的投递链接指向新浪财经（finance.sina.com.cn），用户无法投递。
  3. **🟡 岗位同质化**：10 个适配岗位中 5 个来自百度（50%），全部源自同一条 AIDU 事件。
  4. **🟡 面经问题缺公司特异性**：8 个问题偏通用（"产品经理最重要的是什么？"），缺少针对焦点公司（腾讯/阿里/百度/美团）的差异化面经。
  5. **E2E 覆盖不足**：仅 1 条 mock happy path，缺失档案个性化变化、进度文案、缓存复用、Settings 面板持久化、@real E2E。
- **调整**：
  1. **事件过期三层防线**：
     - 搜索层：`buildEventQueries` 加 `2026秋招 2027届` 年份锚点。
     - Prompt 层：`extract-events.md` 注入 `{{today}}` 占位符，强化时效规则（明确"今天是 {{today}}"+"面向 2025 届及更早的必须丢弃"）。
     - 代码层：新增 `filterAndCapEvents()`——关注公司过滤 + 90 天日期裁剪 + 日期倒序排序 + Top 20 上限。
  2. **投递链接校验**：`match-jobs.md` 新增规则 7（URL 域名特征标注）；`synthesize.md` 新增规则 10（标注媒体报道链接）。
  3. **同公司上限**：`generateJobBriefing` 中 `matchedJobs` 排序后加 diversity 约束（同公司 ≤3 个，总计 Top 10）。
  4. **面经公司特异性**：`aggregate-questions.md` 新增规则 8（优先保留与查询公司直接相关的问题）。
  5. **E2E 补 5 条**：档案填写→提示条消失+state.json 持久化、进度指示器、缓存复用（二次生成不改写）、Settings 档案五字段保存验证、@real @unstable 真实 API 链路。
- **结果**：全量单测 28/28（新增 `filterAndCapEvents` 4 条 + `event query year anchors` 1 条）+ 渲染器单测 7/7 + 全量回归 514 passed（1 条 pre-existing flaky 超时无关）；tsc 零错误。
- **待后续迭代观察**：三层防线 + prompt 强化后真实 API 生成产物质量（`REAL_API=1` 重跑对比迭代 1 fixture）；@real E2E 在 CI 环境稳定性；`{{today}}` 注入对 LLM 时效过滤的实际遵从率。
