...
博客（Anthropic 博客）文章越积越多，用户无法判断哪些值得读。写作仓库（`writing/`）里有用户当前境况的上下文。需求：博客基于写作上下文自动推荐"既至关重要、又切合当前认知缺口"的本地已导入文章，推荐结果挂载进一个内置收藏夹；推荐时先用网络搜索探明"哪些 Anthropic 系文章至关重要"，再在本地文章里匹配。同时把写作的「对照文」交互推广到博客/前沿/求职栏目，统一三栏目的右栏为「导读 | 对照」两 tab，旁注维持现有设计。
...
- 模块 2：博客/前沿/求职右栏统一为「导读 | 对照」两 tab（旁注维持现有设计，不作第三个 tab）；对照 tab 下点击该栏目列表文章 = 切换对照文、主文阅读器正文不变；对照文 md 可编辑（autosave）、html 只读；求职栏目获得导读。
...
- 收藏夹的多级分组/排序/导出。
- 收藏夹的多级分组/排序/导出；推荐历史的清理入口（历史只增不减，容量上限内滚动保留）。
...
- 新一批推荐：entries = 保留 manual + 全量替换 recommend；新批次追加进 `history`（最新在前，容量上限 ~20 批滚动保留），`batch` 递增。
...
5. **阶段三 · 精排**（chatNonStream，prompt `blog-recommend-pick-v1.md`）：输入 画像+gaps+搜索命中+候选池编号清单（title+summary+url）→ 输出**不超过 5 条** `[{source_url, reason, gap}]`（宁缺毋滥，不足就返回实际篇数）。校验 `source_url` 必须在池内，非法丢弃；JSON 失败重试一次，仍败 → `LLM_PARSE_ERROR` + `~/.studyparlor/debug/` 落盘。
6. **写回**：替换 recommend 条目、把新批次追加进 `history`（最新在前）、保存 `.collection.json`，事件推送新 collection。
...
- **收藏夹区**（可折叠，默认展开，`data-testid="blog-collection-section"`）：
  ...
  - **固定入口「往期推荐历史」**（有 history 时，`data-testid="blog-recommend-history"`）：点击展开历史批次列表——每批显示「时间 + 用户画像 + N 个缺口 + M 个检索方向」（`searchUsed=false` 时标注"未使用网络搜索"）。始终位于收藏夹区顶部，即使当前无推荐文章也可回看历史画像。
  - 条目行（history 之下，`data-testid` 前缀 `blog-collection-entry-*`）：推荐条目带 💡 与一行理由摘要，点击展开 **reason + gap**；手动条目无标记。hover 显示 × 移除。点击条目 → `openAnthropicReader(filePath)`；打开前 `fs.existsSync` 校验（经 IPC 结果体现），文件已删 → toast + 从夹移除（feature-dev §8）。
...
博客 / 前沿 / 求职右栏统一为 tab 槽位：**导读 | 对照**（复刻写作 `WritingAssistantPanel` 的互斥 tab 模式，但只有两个 tab）。**旁注（AI 对话）维持现有设计**——博客/前沿沿用现有 `ArticleAssistantPanel` 的竖签「旁注」+ `ChatWindow`，求职沿用现有 `JobAssistantPanel` 的聊天面板，都不并入 tab 结构：

- 博客/前沿：现 `ArticleAssistantPanel`（导读 + 旁注竖签）不动，只在其导读槽上方加「导读 | 对照」两个 tab，导读 tab = 现有 GuideSidebar，新增对照 tab。
- 求职：现有 `JobAssistantPanel`（旁注聊天）不动；在聊天之上新增「导读 | 对照」两个 tab（导读 tab = `GuideSidebar` + 打开 assistantSession 时传 `autoGenerateGuide`；guide-v2 管线对 job briefing 正文的适配在实施计划中验证，若需 keying 调整则按 briefing 日期作 key）。求职对照只读。
...
| 字段 | 位置 | 持久化 | 默认 |
|---|---|---|---|
| `articlePanelMode: Record<'anthropic'\|'scout'\|'job', 'guide'\|'companion'>` | store + `StateJson` | ✅ patchState | `{anthropic:'guide', scout:'guide', job:'guide'}`（对齐现状：博客/前沿默认展示导读，求职新增导读后默认也导读） |
...
- E2E `e2e/specs/article-side-panel.spec.ts`：三栏目「导读 | 对照」tab 壳渲染（UI 出口断言 `data-testid="article-panel-tab-*"`）、博客对照切换且主文不变、切主文恢复映射、求职导读 tab 出现。同步 `e2e/source-map.json`。
...
- 空数据：无 catalog / 无本地文章 / 空收藏夹 / 无 history
- 失败：LLM 失败、解析失败、Tavily 失败、写盘失败、文件被外部删除
- 部分成功：搜索失败但本地匹配成功（searchUsed=false）
- 旧数据兼容：无 `.collection.json`、旧 state.json 无新字段
- 跨重启持久化：收藏夹、dismissed、history（往期推荐历史含画像）、articlePanelMode、articleCompanionMap
- 取消/重入：推荐中取消、推荐中重复点击
- UI 出口：收藏夹区、为我推荐按钮、往期推荐历史入口、☆/★、两 tab 均有 data-testid 且有 e2e 断言
