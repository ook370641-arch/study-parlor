...
> **2026-08-09 修订（用户决策）**：回收站设计下线。删除 = 确认后**真删**（UI 已有确认对话框，用户认为足够）；学习库残留 `.trash/` 已清除，扫描隐藏名单保留该行以防老库残留显示成分组。原「移入 .trash 可回溯」条款作废。

- `writing:delete`：文件 → `deleteNode` 真删；分组 → `dissolveGroup`（组内文章含 `.assistant.md` 等伴生文件全部移回父级，**不会被删除**；空壳目录真删）。
- 删文章确认文案："确定删除《xx》？文件将被永久删除，无法恢复。"
- 删分组确认文案："确定解散分组「xx」？组内 N 篇文章将移回上一级，不会被删除。"
- 复用现有 `ConfirmDialog`。
...
- 不支持 H4-H6。
- 不做跨主题独立配色记忆（默认色跟主题走）。
- ~~不做回收站的 App 内恢复 UI~~（2026-08-09 修订：回收站设计整体下线，删除=确认后真删）。
