...
新增/更新 `e2e/specs/briefing-ux-optimization.spec.ts`，覆盖本次优化：

#### 顶部栏一致性

| # | 场景 | 前置 | 关键断言 |
|---|------|------|---------|
| 1 | 未生成/加载中即见全部按钮 | 删除当日缓存后进入 Briefing | Header 可见 A-、A+、重新生成、往期、主题切换 |
| 2 | 出错时按钮仍在 | 使用返回 500 的本地 feed URL | 错误页 Header 仍可见上述按钮，且可点击「重新生成」 |
| 3 | 往期按钮始终可点 | 任意状态 | 点击「往期」打开抽屉，不依赖是否已生成 |
| 4 | 换画按钮不挤占 Header | 学术版式 | 换画按钮位于内容区左上角，Header 高度与报纸版一致 |

#### 字号调节

| # | 场景 | 前置 | 关键断言 |
|---|------|------|---------|
| 5 | 默认 base 字号 | 新用户或重置 state | 正文 CSS 变量对应 15px |
| 6 | A+ 增大字号 | 点击 A+ 两次 | 字号档位从 base → lg → xl，内容区字体明显变大 |
| 7 | A- 减小字号并边界 | 点击 A- 到最小 | 档位到 sm 后 A- 禁用；再次进入仍保持 sm |
| 8 | 字号跨版式共享 | 在学术版调到 xl，切换到报纸版 | 报纸版正文同样为 xl |

#### 信息精简

| # | 场景 | 前置 | 关键断言 |
|---|------|------|---------|
| 9 | 顶部无「AI 行业日报」 | 生成后 | 顶部不出现「AI 行业日报」字样 |
| 10 | 时间无「生成于」 | 生成后 | 时间显示为 `08:42` 或 `2026 年 7 月 5 日 · 08:42` |
| 11 | 正文无装饰性刊头 | 生成后 | 内容区不出现 `AI Builders Digest`、`Vol.`、`档案编号`、`Briefing`、`学习卷宗` |

#### Feed 稳定性与源状态

| # | 场景 | 前置 | 关键断言 |
|---|------|------|---------|
| 12 | 博客源失败仍可生成 | 将博客 feed URL 指向空数组或 500 | 简报生成成功，顶部显示 `博客 ✗` |
| 13 | 全部 feed 失败显示网络错误 | 三个 feed URL 均指向 500 | 显示网络错误与重试按钮，不显示 FEED_EMPTY |
| 14 | 空 feed 仍触发 FEED_EMPTY | 三个 feed 均返回空数组 | 显示「今日海面平静，暂无新信号」 |
| 15 | 重试后成功 | 第一次返回 500、第二次返回正常内容的本地 server | 最终生成成功，sourceStatus 全为 ok |

#### 内容深度

| # | 场景 | 前置 | 关键断言 |
|---|------|------|---------|
| 16 | 中文段落含小白解释 | 真实或 mock LLM 生成 | 至少一条 builder/blog/podcast 的中文段落出现零基础解释句 |
| 17 | 无底部版权行 | 生成后 | 内容末尾不出现 `Generated through the Follow Builders skill` |

> 注：以上涉及真实 LLM 生成的用例标记 `@unstable`；使用本地 mock server 的用例标记 `@smoke`。
