---
name: general
description: 跨领域 Agent 行为守则（每个任务都适用）：边界/null/错误校验、跨层契约同步、简单优先、prompt 契约稳定、测试超快乐路径、打包环境、异步/取消生命周期、状态向后兼容、只跑受影响测试。
whenToUse: 每个任务都适用；尤其涉及外部数据、跨层改动、异步/持久化、打包或测试时使用。
---
