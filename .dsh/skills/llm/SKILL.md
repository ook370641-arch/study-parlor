---
name: llm
description: LLM 集成编码规则：prompt 契约、JSON 输出提取/消毒、流式与 abort、归档 finalize。改 electron/lib/kimi.ts、electron/ipc/llm.ts、electron/prompts/**、electron/lib/llm-tasks.ts、src/lib/finalize.ts、src/lib/session-runtime.ts 前加载。
whenToUse: 调用 LLM、写 prompt、解析 JSON 输出、处理会话归档时使用。
---
