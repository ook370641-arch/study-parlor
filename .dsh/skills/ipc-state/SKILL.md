---
name: ipc-state
description: IPC/状态编码规则：跨四层同步、错误码、state.json 兼容、frontmatter/DocType 全链路。改 src/types/**、src/store/**、src/lib/ipc.ts、electron/ipc/**、electron/preload.ts、electron/lib/frontmatter.ts、electron/lib/app-paths.ts 前加载。
whenToUse: 新增 IPC handler、state 字段、frontmatter schema 或 DocType 时使用。
---
