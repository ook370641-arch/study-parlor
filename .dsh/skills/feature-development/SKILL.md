...
description: 功能开发规则：先锁 spec 与验收清单、外部 API 凭据隔离、错误码映射、幂等写入、取消/超时/重试、UI 出口声明。改 src/**、electron/**、docs/superpowers/specs/** 新增功能前加载。
whenToUse: 添加功能、集成外部 API、定义持久化/交付边界时使用。
---
