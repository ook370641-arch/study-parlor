---
name: ui-styling
description: UI/样式规则：Chrome 解耦、状态指示器、抽屉/动画边界、markdown 外链、Fast Refresh、学者夜话设计语言。改 src/components/**、src/pages/**、tailwind.config.ts 前加载。
whenToUse: 构建 React 组件、页面、Tailwind 样式、抽屉、动画或 markdown 渲染时使用。
---
...
- 光的语言：烛光（点照明，CandlelightLayer，screen 混合 alpha ≤0.20）。求职星蓝烛光为例外主色的合法用法（源标识性元素）。聚焦呼吸（区照明）已于 2026-07-26 因真实反馈过暗下线，不再登记。
