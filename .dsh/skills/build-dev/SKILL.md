---
name: build-dev
description: 构建/开发环境规则：asar 资源路径、dev/packaged 可写路径、孤儿进程清理、E2E 隔离、optimizeDeps。改 scripts/**、electron-builder.yml、electron/main.ts、electron/env.ts、electron/lib/app-paths.ts、electron.vite.config.ts、.gitignore 前加载。
whenToUse: 修改构建脚本、electron-builder 配置、开发环境、进程清理或 app 路径时使用。
---
