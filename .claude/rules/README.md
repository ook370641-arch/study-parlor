...
- **粒度约束**：Why 限一句话（不加叙事/数据）；checklist 3-5 条，每条一行（不嵌套代码块）；Example 只在抽象概念需要具体化时加。新增规则与已有条目粒度对齐。
...
- `2026-07-25` feature-development 新增 §12：功能必须声明 UI 出口（来自写作助手仅 24px 竖条入口、用户以为功能不存在的反馈）。
