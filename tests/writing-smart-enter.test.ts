...
// （含 baseKeymap 的 splitBlock / 列表 Enter）处理按键后都会调用 preventDefault，
// 因此 defaultPrevented 在交还默认时恒为 true，无法区分"smart-enter 未拦截"。
// 改为用结果文档结构断言"默认行为确实发生"（段尾→2 段、列表→3 项、空段→splitBlock）。
